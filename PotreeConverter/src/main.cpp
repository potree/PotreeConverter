...
int main(int argc, char **argv){
	...
	SelectionAlgorithm selectionMode;
	...
	try{
		...
		{// set parameters 
			...
			if(!vm.count("selection-algorithm")) selectionModeString = "FAST";
			if(selectionModeString == "NICE"){
				selectionMode = SelectionAlgorithm::NICE;
			}else if(selectionModeString == "FAST"){
				selectionMode = SelectionAlgorithm::FAST;
			}
		...
		{ // print params
			...
			cout << "selection-algorithm: " << selectionModeString<< endl;
			cout << endl;
		}
	}catch(exception &e){
	...
	try{
		PotreeConverter pc(source, outdir, spacing, diagonalFraction, levels, format, range, scale, outFormat, selectionMode);
		pc.convert();
	}catch(exception &e){
		cout << "ERROR: " << e.what() << endl;
		return 1;
