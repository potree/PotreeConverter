...
namespace indexer_voxels{
	...
	void Indexer::flushChunkRoot(shared_ptr<Node> chunkRoot) {
		...
		int64_t sizePoints = chunkRoot->points ? chunkRoot->points->size : 0;
		int64_t sizeVoxels = chunkRoot->voxels.size() * sizeof(Voxel);

		if(sizePoints > 0){		
			fChunkRoots.write(chunkRoot->points->data_char, sizePoints);
		}
		if(sizeVoxels > 0){
			fChunkRoots.write(reinterpret_cast<const char*>(&chunkRoot->voxels[0]), sizeVoxels);
		}

		FlushedChunkRoot fcr;
		fcr.node = chunkRoot;
		fcr.offset = offset;
		fcr.sizePoints = sizePoints;
		fcr.sizeVoxels = sizeVoxels;

		chunkRoot->points = nullptr;
		chunkRoot->voxels.clear();

		flushedChunkRoots.push_back(fcr);

		offset += sizePoints + sizeVoxels;
	}
	...
	void Indexer::reloadChunkRoots() {
		...
		struct LoadTask {
			...
			LoadTask(shared_ptr<Node> node, int64_t offset, int64_t sizePoints, int64_t sizeVoxels) {
				this->node = node;
				this->offset = offset;
				this->sizePoints = sizePoints;
				this->sizeVoxels = sizeVoxels;
			}
		};
		...
		TaskPool<LoadTask> pool(16, [targetDir](shared_ptr<LoadTask> task) {
			...
			if(task->sizePoints > 0){
				auto buffer = make_shared<Buffer>(task->sizePoints);
				readBinaryFile(octreePath, start, task->sizePoints, buffer->data);
				node->points = buffer;
			}

			if(task->sizeVoxels > 0){
				auto bufferVoxels = make_shared<Buffer>(task->sizeVoxels);
				readBinaryFile(octreePath, task->offset + task->sizePoints, task->sizeVoxels, bufferVoxels->data);
				int numVoxels = task->sizeVoxels / sizeof(Voxel);
				node->voxels.resize(numVoxels);
				memcpy(&node->voxels[0], bufferVoxels->data, task->sizeVoxels);
			}
			
		});

		for (auto fcr : flushedChunkRoots) {
			auto task = make_shared<LoadTask>(fcr.node, fcr.offset, fcr.sizePoints, fcr.sizeVoxels);
			pool.addTask(task);
		}
...
void doIndexing(string targetDir, State& state, Options& options, Sampler& sampler) {
	...
	auto writeAndUnload = [&indexer, &attributes](Node* node) {
		indexer.writer->writeAndUnload(node, attributes);
	};
	...
	TaskPool<Task> pool(numSampleThreads(), [&writeAndUnload, &state, &options, &activeThreads, tStart, &lastReport, &totalPoints, totalBytes, &pointsProcessed, chunks, &indexer, &nodes, &mtx_nodes, &sampler](auto task) {
		...
		auto onNodeCompleted = [&indexer, &attributes](Node* node) {
			indexer.writer->writeAndUnload(node, attributes);
		};
	...
	indexer.writer->writeAndUnload(indexer.root.get(), attributes);
